import json
import os
from Modified_ConEx.helper_classes import *
from Modified_ConEx.helper_funcs import *

from collections import defaultdict
from Modified_ConEx.models import *
...
class Experiment:
    ...
    def get_batch(self, d, er_vocab, er_vocab_pairs, idx):
    ...
    def evaluate(self, model, d, data):
        hits = []
        ranks = []
        for i in range(10):
            hits.append([])
        ...
        for i in range(0, len(test_data_idxs), self.batch_size):
            data_batch, _ = self.get_batch(d, er_vocab, test_data_idxs, i)
    ...
    def train_and_eval(self, d, d_info, show_every=10):
        ...
        print("Info pertaining to dataset:{0}".format(d_info['dataset']))
        print("Number of triples in data:{0}".format(len(d.train_data)))
        #print("Number of triples in validation data:{0}".format(len(d.valid_data)))
        #print("Number of triples in testing data:{0}".format(len(d.test_data)))
        ...
        for it in range(1, self.num_iterations + 1):
            ...
            for j in range(0, len(er_vocab_pairs), self.batch_size):
                data_batch, targets = self.get_batch(d, er_vocab, er_vocab_pairs, j)
                opt.zero_grad()
                e1_idx = torch.tensor(data_batch[:, 0])
                r_idx = torch.tensor(data_batch[:, 1])
                if self.cuda:
            ...
            if it % show_every == 0:
                print('Iteration:{0} with Average loss{1}'.format(it, np.mean(losses)))
                model.eval()  # Turns evaluation mode on, i.e., dropouts are turned off.
                with torch.no_grad():  # Important:
                    # if len(d.valid_data) > 1:
                    #     print("Validation:")
                    #     self.evaluate(model, d, d.valid_data)
                    
                    print("Train:")
                    self.evaluate(model, d, d.train_data)
                model.train()

        with open(self.storage_path + '/settings.json', 'w') as file_descriptor:
            json.dump(self.kwargs, file_descriptor)
        # if len(d.test_data)>1:
        #     print("Testing:")
        #     self.evaluate(model, d, d.test_data)
        self.Model = model
...
if __name__ == '__main__':
    ...
    dataset = "../OntoPy/data/Family_Benchmark"#args.dataset
    data_dir = "%s/" % dataset
    ...
    experiment.train_and_eval(d, d.info)
